...
class SmartCache:
    """Redis-based caching service with intelligent cache management and memory fallback"""
    
    def __init__(self):
        self.redis_client: Optional[redis.Redis] = None
        self.memory_cache = MemoryCache(max_size=1000)
        self._lock = asyncio.Lock()
        self._redis_available = None  # None=unknown, True=available, False=unavailable
    ...
    async def _should_attempt_redis(self) -> bool:
        """Check if we should attempt Redis connection based on recent failures"""
        # If Redis is explicitly disabled, never attempt
        if self._redis_available is False:
            return False
            
        # If Redis client is None, don't attempt
        if self.redis_client is None:
            return False
    ...
    async def delete_pattern(self, pattern: str) -> int:
        ...
        # Clear all memory cache when pattern delete is requested
    ...
    async def health_check(self) -> bool:
        """Check if cache is healthy (either Redis or memory)"""
        if await self._should_attempt_redis():
            try:
                self.redis_client.ping()
                return True
...
class RAGCache:
    ...
    async def invalidate_rag_cache(self) -> int:
        """Invalidate all RAG-related cache entries"""
